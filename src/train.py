...
from .utils import load_config, optimizers_map, get_optimizer, get_loss, get_dataset
...
parser.add_argument("--config", "-c", dest="config", type=str, help="config file")
parser.add_argument(
    "--device",
    dest="device",
    type=int,
...
parser.add_argument(
    "--logs", "-l", dest="logdir", type=str, help="directory to store logs"
)
parser.add_argument(
    "--checkpoint",
    "-s",
...
def main() -> None:
    ...
    runner.train(
        ...
        callbacks=[
            ...
            ),  # accumulate gradients of 4 batches
